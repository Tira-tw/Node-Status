...
module.exports = function checkStatus({ client }) {
	...
	function Embed({ node }) {
		return new EmbedBuilder()
			.setTitle('所有節點正在讀取中') //if you wanted to change this please change at line 175 too
			.setDescription('`' + node.name + '` 下線中')
			.setFooter({ text: '請查看後台' })
			.setTimestamp()
			.setColor('ED4245')
	}

	if (client.config.channel.startsWith('Put')) {
		console.log(chalk.cyan('Lesa Host') + chalk.red('頻道ID錯誤'))
		process.exit()
	} else if (client.config.panel.url.startsWith('Put')) {
		console.log(chalk.cyan('Lesa Host') + chalk.red('面板連結錯誤'))
		process.exit()
	} else if (client.config.panel.key.startsWith('Put')) {
		console.log(chalk.cyan('Lesa Host') + chalk.red('面板Apikey錯誤'))
		process.exit()
	} else if (!client.config.panel.url.startsWith('http')) {
		console.log(chalk.cyan('Lesa Host') + chalk.red('面板連結錯誤'))
		console.log(chalk.cyan('Lesa Host') + chalk.red('面板連結要包含"https://" or "http://"'))
		process.exit()
	}
	...
	console.log(chalk.cyan('Lesa Host') + chalk.green('開始檢查節點'))
		...
		axios(client.config.panel.url + '/api/application/users', {
			...
			if (error.response) {
				if (error.response.status === 403) {
					console.log(chalk.cyan('Lesa Host') + chalk.red('面板Apikey錯誤'))
					console.log(chalk.cyan('Lesa Host') + chalk.red('1.確定您的apikey不是使用user apikey , 需要使用Application API apikey'))
					console.log(chalk.cyan('Lesa Host') + chalk.red('2. 確定您的apikey權限只有閱讀!'))
					console.log(chalk.cyan('Lesa Host') + chalk.red('3. 確定apikey是否正確?'))
				} else if (error.response.status === 404) {
					console.log(chalk.cyan('Lesa Host') + chalk.red('面板連結錯誤'))
					console.log(chalk.cyan('Lesa Host') + chalk.red('1. 確定您的面板連結是否有誤\n 正確 :\n "https://panel.thisalsea.eu.org"'))
				} else console.log(chalk.cyan('Lesa Host') + chalk.red('錯誤la ' + error))
			} else console.log(chalk.cyan('Lesa Host') + chalk.red('錯誤la ' + error))
			resolve()
		})
	})
				...
				axios(client.config.panel.url + '/api/application/nodes/' + node.attributes.id + '/configuration', {
					...
					const stats = new Promise((statsResolve, statsReject) => {
						axios(node.attributes.scheme + '://' + node.attributes.fqdn + ':' + node.attributes.daemon_listen + '/api/servers', {
							...
							if (client.config.log_error) console.log(chalk.cyan('Lesa Host') + chalk.yellow('[節點: ' + node.attributes.name + '] ') + chalk.red(error))
						...
						setTimeout(() => {
							if (body.status === undefined) {
								if (client.config.log_error) console.log(chalk.cyan('Lesa Host') + chalk.yellow('[節點: ' + node.attributes.name + '] ') + chalk.red('停止'))
								embeds.push(Embed({ node: body }))
								body.status = false
								return statsResolve()
							}
		...
		nodeStats.then(async () => {
			...
			if (client.config.mentions.user.length > 0 || client.config.mentions.role.length > 0 && client.config.mentions.channel) {
				if (Array.isArray(client.config.mentions.user) || Array.isArray(client.config.mentions.role)) {
					...
					if (channel) {
						const messages = await channel.messages.fetch({ limit: 10 }).then(msg => msg.filter(m => m.author.id === client.user.id && m.embeds[0].data.title === '所有節點正在讀取中').first())
