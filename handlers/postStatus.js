...
module.exports = async function postStatus({ client, panel, nodes }) {
	...
	if (!channel) return console.log(chalk.cyan('Lesa Host') + chalk.red('頻道ID錯誤'))
	...
	if (client.guilds.cache.size < 1) return console.log(chalk.cyan('Lesa Host') + chalk.red('你要確定你的bot有沒有在伺服器a owo'))
					...
					switch (client.config.nodes_settings.unit.toLowerCase()) {
						case 'tb':
							description = description +
								'\n記憶體 : ' + Math.floor(data.memory_min / (1024 * 1000)).toLocaleString() + ' TB / ' + Math.floor(data.memory_max / (1024 * 1000)).toLocaleString() + ' TB' +
								'\n儲存空間 : ' + Math.floor(data.disk_min / (1024 * 1000)).toLocaleString() + ' TB / ' + Math.floor(data.disk_max / (1024 * 1000)).toLocaleString() + ' TB'
							break;
						case 'gb':
							description = description +
								'\n記憶體 : ' + Math.floor(data.memory_min / 1024).toLocaleString() + ' GB / ' + Math.floor(data.memory_max / 1024).toLocaleString() + ' GB' +
								'\n儲存空間 : ' + Math.floor(data.disk_min / 1024).toLocaleString() + ' GB / ' + Math.floor(data.disk_max / 1024).toLocaleString() + ' GB'
							break;
						case 'percent':
							description = description +
								'\n記憶體 : ' + Math.floor(data.memory_min / data.memory_max * 100) + ' %' +
								'\n儲存空間 : ' + Math.floor(data.disk_min / data.disk_max * 100) + ' %'
							break;
						default:
							description = description +
								'\n記憶體 : ' + data.memory_min.toLocaleString() + ' MB / ' + data.memory_max.toLocaleString() + ' MB' +
								'\n儲存空間 : ' + data.disk_min.toLocaleString() + ' MB / ' + data.disk_max.toLocaleString() + ' MB'
					}

					if (client.config.nodes_settings.servers) description = description + '\n伺服器 : ' + data.total_servers.toLocaleString()
					if (client.config.nodes_settings.location) description = description + '\n地區 : ' + data.location
					if (client.config.nodes_settings.allocations) description = description + '\n分配 : ' + data.allocations.toLocaleString()
	...
	stats.then(async () => {
		...
		embed.setDescription(desc.replaceAll('{{time}}', format) + '\n**免費節點狀態 [' + Math.floor(nodes.length - blacklist) + ']**' + text)
		const EmbedFields = []

		if (client.config.panel_settings.status) {
			let stats = '**節點狀態:** ' + String(panel.status).replace('true', client.config.status.online).replace('false', client.config.status.offline) + '\n\n'

			if (client.config.panel_settings.users) stats = stats + '用戶: ' + String(panel.total_users).replace('-1', '`Unknown`') + '\n'
			if (client.config.panel_settings.servers) stats = stats + '伺服器: ' + String(panel.total_servers).replace('-1', '`Unknown`')

			EmbedFields.push({ name: '免費節點狀態', value: stats })
		}
		...
		console.log(chalk.cyan('Lesa Host') + chalk.green('已發送到頻道owo'))
	})
}